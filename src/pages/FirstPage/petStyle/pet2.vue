  ...
  <el-tabs type="border-card">
    ...
    <el-tab-pane>
      ...
      <div class="record">
        ...
        type="text" 
        @click="appear($event)" 
        id="tijao">保 存 你 的 故 事</el-button>
    </template>
    ...
    <el-tab-pane>
      ...
      <div class="check">
        ...
        v-if=panduan>
          ...
          <!-- <button >返回上一级</button> -->
          <div class="petContentSide" ref="petCheck">
          </div>
          </div>
        <ul  class="petshowul" v-else >
          <!-- 通过v-for来渲染页面，绑定点击事件，同时把index索引值作为参数传过去 -->
          <!-- 如果故事为空的话显示下面的内容 -->
           <el-empty description="还 未 添 加 故 事 , 请 添 加 故 事 后 再 来 查 看 !" 
           v-show='this.petStory==""'
           :image-size="300"
           ></el-empty>
          <li class="petshowlis" 
           v-for="(petS,index) in this.petStory" 
           :key='index' 
           @dblclick="showStory(index)">
            {{value[index]}}
            <el-button 
            type="danger" 
            icon="el-icon-delete" 
            id="delbtn"
            @click="delStory(index)"></el-button>
          </li>
  ...
  export default {
    ...
    data() {
      return {
        ...
        value : '',
        content: {
          story: '<h3>请书写你的故事，记录下那些你与动物的难忘时光!<h3>'
        },
    ...
    methods: {
      ...
      appear() {
         this.$prompt('请为您的故事命名!', '故事名', {
          ...
          this.editorOption.story = '<h3>请书写你的故事，记录下那些你与动物的难忘时光!<h3>'
...
<style>
  ...
  /* 让标题居中显示 */
  .petContentSide h1,h2,h3,h4,h5,h6:first-of-type {
    text-align: center;
    margin-bottom: 20px;
  }
  .petContentSide p {
